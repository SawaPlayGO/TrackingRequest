...
async def create_ticket(
    ...
    try:
        response_ticket: TicketResponse = await ticket_service.create(uow=uow, ticket=ticket)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"status": "error"})
    except Exception:
...
async def get_tickets_list(
    ...
    try:
        result: TicketListResponse = await ticket_service.get_list(uow=uow, params=params)
    except Exception:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail={"status": "error"})
    return result
