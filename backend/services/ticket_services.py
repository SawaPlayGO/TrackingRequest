...
from schemes.tickets import TicketCreate, TicketListResponse, TicketPaginationQueryParams, TicketResponse, TicketStatusUpdate
...
class TicketService:
    ...
    async def create(uow: IUnitOfWork, ticket: TicketCreate) -> TicketResponse:
        async with uow:
            new_ticket: Ticket | None = await uow.ticket_repo.create(title=ticket.title, status=ticket.status, priority=ticket.priority, description=ticket.description)
    ...
    @staticmethod
    async def get_list(uow: IUnitOfWork, params: TicketPaginationQueryParams) -> TicketListResponse:
        ...
        async with uow:
            tickets_models = await uow.ticket_repo.get_list(
                ...
                sort_by_priority=params.sort_by_priority.value if params.sort_by_priority else None,
        ...
        ticket_responses = [TicketResponse.model_validate(ticket) for ticket in tickets_models]
    ...
    @staticmethod
    async def update_status(uow: IUnitOfWork, ticket_id: int, payload: TicketStatusUpdate) -> TicketResponse:
        async with uow:
    ...
    async def delete_ticket(uow: IUnitOfWork, ticket_id: int) -> None:
        async with uow:
            ...
            if ticket.status == TicketStatus.DONE:
                await logger.aerror(f"Cannot delete a ticket with ID {ticket_id} that is already DONE")
    ...
    @staticmethod
    def _validate_status_transition(current_status: TicketStatus, new_status: TicketStatus) -> None:
