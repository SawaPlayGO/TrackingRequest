...
class TicketPaginationQueryParams(BaseModel):
    ...
    sort_by_date: SortTicket = Field(SortTicket.DESC, description="Сортировка по дате создания (asc/desc)")
    sort_by_priority: Optional[SortTicket] = Field(None, description="Сортировка по приоритету (asc/desc). Если не указана, не применяется.")

    search: Optional[str] = Field(None, min_length=1, description="Поиск по названию или описанию заявки")

class TicketListResponse(BaseModel):
    items: list[TicketResponse] = Field(..., description="Список заявок на текущей странице")
    
    total: int = Field(..., description="Общее количество заявок в базе по заданным фильтрам")
